...
def test_conversational_memory() -> bool:
    ...
    payload1 = {
        "query": "hi, my name is Ahmed",
        "session_id": session_id
    }
    try:
        resp1 = requests.post(API_URL, json=payload1, timeout=30)
        resp1.raise_for_status()
        data1 = resp1.json()
        print(f"  Turn 1 Query: {payload1['query']}")
    ...
    try:
        resp2 = requests.post(API_URL, json=payload2, timeout=30)
        resp2.raise_for_status()
        data2 = resp2.json()
        ans2 = data2.get("answer", "")
        print(f"  Turn 2 Query: {payload2['query']}")
...
def test_mode_switching() -> bool:
    ...
    payload1 = {
        "query": "hello, I am looking for some help",
        "session_id": session_id
    }
    try:
        resp1 = requests.post(API_URL, json=payload1, timeout=30)
    ...
    payload3 = {
        "query": "thanks, that is clear. How are you doing today?",
        "session_id": session_id
    }
    try:
        resp3 = requests.post(API_URL, json=payload3, timeout=30)
        resp3.raise_for_status()
        data3 = resp3.json()
        print(f"  Turn 3 Query: {payload3['query']}")
...
def main():
    ...
    if mem_pass and switch_pass and scope_pass:
